...
class Execution:
    ...
    def get_user_logs(
        ...
        *,
        ...
        Uses :meth:`deeporigin.platform.user_logs.UserLogs.search` with the
        execution UUID (same value as :meth:`get_results`'s ``compute_job_id``),
        which the API applies as an ``execution_id`` filter on ``user_logs``.
