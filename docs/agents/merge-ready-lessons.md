...
## 2026-08-31 — PR #621 — PocketFinder define-by-selection

SonarCloud quality gate failed on real issues, not flakes: a duplicated module-path string literal (S1192, threshold 3+ dupes) and two functions over cognitive-complexity-15 (S3776) — extracting one small helper per validation branch fixed both cleanly. Copilot then caught, over two review rounds, that this class's `__init__` (unlike every sibling `Execution` subclass — `admet.py`, `metabolism.py`, `protein_prep.py`, etc. — all decorate `__init__` with `@beartype`) had no type enforcement at the boundary: `mode`/`kind` were checked with `in <frozenset>` without confirming they were strings first (TypeError on non-hashable input instead of a clean ValueError), `align_to_pocket` was silently coerced with `bool(...)`, and DTO-parsing used `x = inputs.get(k) or default` — which also swallows a **falsy-but-invalid** stored value (`""`, `[]`) into the default instead of surfacing it as corrupt. Fix pattern: `raw = inputs.get(k); if raw is None: raw = default` (only `None` means "absent"), plus `isinstance` guards before every `in <set>` membership check on externally-sourced values.
