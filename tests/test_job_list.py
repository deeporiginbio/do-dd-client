...
def test_job_duplicate_success():
    """Test successful job duplication."""
    # Create a job with all required attributes
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
        "userInputs": {"param1": "value1"},
        "userOutputs": {"output1": "result1"},
        "orgKey": "test-org",
        "tag": "test-tag",
        "tool": {"key": "deeporigin.abfe-end-to-end", "version": "0.2.19"},
    }

    # Mock client and tools
    mock_client = MagicMock(spec=DeepOriginClient)
    mock_tools = MagicMock()
    mock_client.tools = mock_tools

    # Mock the response from tools.run()
    response_dto = {
        "executionId": "new-id-1",
        "status": "Created",
        "tool": {"key": "deeporigin.abfe-end-to-end", "version": "0.2.19"},
    }
    mock_tools.run.return_value = response_dto
    ...
    # Verify tools.run was called with correct parameters
    mock_tools.run.assert_called_once_with(
    ...
    # Verify new job was created from DTO
    assert isinstance(new_job, Job)
    assert new_job._id == "new-id-1"
    assert new_job.status == "Created"


def test_job_duplicate_with_minimal_fields():
    """Test duplication with only required fields."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
    ...
    # Verify only tool was passed (other fields are optional)
    mock_tools.run.assert_called_once_with(
        tool_key="deeporigin.docking",
        tool_version="1.0.0",
        data={"tool": {"key": "deeporigin.docking", "version": "1.0.0"}},
    )

    assert isinstance(new_job, Job)
    assert new_job._id == "new-id-2"


def test_job_duplicate_missing_attributes():
    """Test duplication fails when attributes are missing."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = None

    mock_client = MagicMock(spec=DeepOriginClient)
    job.client = mock_client

    with pytest.raises(
        ValueError, match="Cannot duplicate job: job attributes are not available"
    ):
        job.duplicate()


def test_job_duplicate_missing_tool():
    """Test duplication fails when tool is missing."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
        "userInputs": {"param1": "value1"},
    ...
    with pytest.raises(
        ValueError, match="Cannot duplicate job: tool information is missing or invalid"
    ):
        job.duplicate()


def test_job_duplicate_missing_tool_key():
    """Test duplication fails when tool key is missing."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
        "tool": {"version": "0.2.19"},  # Missing key
    }
    ...
    with pytest.raises(
        ValueError, match="Cannot duplicate job: tool key or version is missing"
    ):
        job.duplicate()


def test_job_duplicate_missing_tool_version():
    """Test duplication fails when tool version is missing."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
        "tool": {"key": "deeporigin.abfe-end-to-end"},  # Missing version
    }
...
def test_job_duplicate_client_auto_initialization():
    """Test that duplicate() initializes client if None."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
        "tool": {"key": "deeporigin.abfe-end-to-end", "version": "0.2.19"},
    ...
    response_dto = {"executionId": "new-id-3", "status": "Created"}
    mock_tools.run.return_value = response_dto

    with patch(
        "deeporigin.platform.job.DeepOriginClient.get", return_value=mock_client
    ):
        new_job = job.duplicate()
...
def test_job_duplicate_excludes_other_fields():
    """Test that duplicate() only includes allowed fields."""
    job = Job(name="job1", _id="id-1", _skip_sync=True)
    job._attributes = {
        "executionId": "id-1",
    ...
    response_dto = {"executionId": "new-id-4", "status": "Created"}
    mock_tools.run.return_value = response_dto
    ...
    assert "userOutputs" in data
    assert "orgKey" in data
    assert "tag" in data
    assert "tool" in data
    ...
    assert "progressReport" not in data
